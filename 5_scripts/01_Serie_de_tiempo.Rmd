...
setwd("~/Desktop/22 Migración ASI/23 Gráficas nuevas")
...
bd_ocupacion <- readRDS("bd_ocupacion.rds")
bd.ocupacion.jov <- readRDS("bd_ocupacion_jov.rds")

bd.ocupacion.ts <- bd_ocupacion %>% 
...
n_mig <- bd.ocupacion.ts %>% filter(fecha < 2022) %>% select(Total) %>% ts(frequency = 4, start= c(2005,3))
...
```{r}
tseries::adf.test(l_n_mig,alternative = "stationary")
```


```{r cars}

acf(l_n_mig, lag.max=30)
pacf(l_n_mig, lag.max=30)
...
modelo <- arima(l_n_mig, order=c(2,1,1))
