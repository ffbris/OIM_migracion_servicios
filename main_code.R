...
pacman::p_load(tidyverse, ggplot2) # Common use
...
common_params <- theme(
  ...
  axis.text = element_text(size = 12, family = "Gill Sans Nova Book"),
  plot.title = element_text(size = 16, family = "Gill Sans Nova Book", hjust = 0.5),
  plot.subtitle = element_text(size = 14, family = "Gill Sans Nova Book", hjust = 0.5)
)
